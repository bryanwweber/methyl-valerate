...
relative to conventionally fueled engines~\cite{Hoekman2012}. A recent
review paper summarizes the work on methyl esters relevant to biodiesel
combustion~\cite{Coniglio2013}. Autoignition of methyl butanoate (MB,
...
\cite{Coniglio2013} for further details. The prevalence of MB data in the
literature is largely due to the early identification of MB as a potential
surrogate fuel for biodiesel~\cite{Fisher2000}. However, the experiments
...
suitable biodiesel surrogate~\cite{Coniglio2013}.
...
\cite{Hadj-Ali2009} used a rapid compression machine (RCM) to study the
...
its oxidation. \cite{Korobeinichev2015} studied MV in premixed laminar
flames and extended a detailed high temperature chemical kinetic model to
include MV and methyl hexanoate. \cite{Dmitriev2015} added MV to
...
vinylacetylene~\cite{Dmitriev2015}. \cite{Hayes2009} computationally
examined the peroxy radical isomerization reactions for MV to better understand
the low temperature reaction pathways. Finally, \cite{Dievart2013} used
...
previously~\cite{Mittal2007a} and will be described here briefly for
...
pressure. The pressure data is processed by a Python package called UConnRCMPy~\cite{uconnrcmpy}, which calculates \(P_C\), \(T_C\), and the ignition
...
relations between pressure and temperature~\cite{Lee1998}. The EOC
...
the data of \cite{Ortega2003}, \cite{vanGenderen2002}, and
\cite{Verevkin2008} using the \verb|curve_fit()| function of
SciPy~\cite{Jones2001} version 0.18.1. \Cref{fig:vapor-pressure} shows that
...
%         \cite{Ortega2003} & 6.23175 & 1429.00 & 62.30 & 364.75 & 417.18 \\
%         \cite{Camacho2007} & 5.9644 & 1281.06 & 75.94 & 281 & 547 \\
%         \cite{Stephenson1987} & 6.62646 & 1658.4 & 42.09 & 297 & 411 \\
%         \bottomrule
%     \end{tabular}
% \end{table}
...
The Python 3.5 interface of Cantera~\cite{cantera} version 2.2.1 is used for
all simulations in this work. Detailed descriptions of the use of Cantera for
these simulations can be found in the work of \cite{Weber2016a} and
\cite{Dames2016}; a brief overview is given here. As mentioned in
...
in an \verb|IdealGasReactor| in Cantera~\cite{cantera} using the CVODES
solver from the SUNDIALS suite~\cite{Hindmarsh2005}. The ignition delay from
...
Lagrange polynomials, as discussed by \cite{Chapra2010}.

To the best of our knowledge, there are three mechanisms for MV combustion
available in the literature. The first two, by \cite{Korobeinichev2015} and
\cite{Dmitriev2015}, were developed to simulate flames, and do not include
the low-temperature chemistry necessary to simulate the conditions in these
experiments. The third model was developed by \cite{Dievart2013} and
...
% used.
The detailed \cite{Dievart2013} model includes 1103 species and 7557
reactions.
...
(RMG)~\cite{Allen2012} version 1.0.4. The Python version of RMG is used,
...
of species is much lower than the \cite{Dievart2013} model because the RMG
...
\cite{Dievart2013} for the $\phi=1.0$ experiments. Results for the other
equivalence ratios are similar to these results, so are not shown here. It is
important to note that the model of \cite{Dievart2013} was not validated for
