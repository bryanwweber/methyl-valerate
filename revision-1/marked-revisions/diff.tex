...
%DIF DEL methyl-valerate_old.tex   Tue Jul 18 13:54:46 2017
%DIF ADD methyl-valerate.tex       Wed Sep 20 21:21:51 2017
...
out to provide additional information on the fidelity of using MV as a biodiesel surrogate \DIFaddbegin \DIFadd{and on
the autoignition chemistry of large methyl esters}\DIFaddend .
...
\begin{center}
    ...
    literature\DIFaddbegin \DIFadd{, in units of \si{\kelvin} and \si{\kPa}}\DIFaddend . The $2\sigma$ confidence interval is
...
\begin{center}
    ...
    a) \(\phi=1.0,\ P_C=\SI{30}{\bar}\), b) \(\phi=2.0,\ P_C=\SI{15}{\bar}\)\DIFdelbegin %DIFDELCMD < }%%%
...
\begin{center}
    ...
    \caption{Comparison of experimental and simulated \DIFdelbegin \DIFdel{results}\DIFdelend \DIFaddbegin \DIFadd{ignition delays computed using the procedure
    described in \mbox{%DIFAUXCMD
\cref{sec:experimental-modeling}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . a) \SI{15}{\bar}, b) \SI{30}{\bar}\DIFdelbegin %DIFDELCMD < }%%%
\DIFdelend .\DIFaddbegin }
...
model appears to be predicting an NTC region of the ignition delays as the temperature decreases, \DIFaddbegin \DIFadd{as
judged by the increasing curvature of the simulations, }\DIFaddend although such a trend is not observed in the
...
\begin{center}
    ...
    \caption{\DIFadd{Comparison of simulated ignition delays computed in an adiabatic, constant volume
...
\DIFadd{To further understand }\DIFaddend the model of \citet{Dievart2013}\DIFdelbegin \DIFdel{over-predicts the first-stage ignition delay, it also
over-predicts the first-stage pressure rise, thereby leading to rapid overall ignition .
}\DIFdelend \DIFaddbegin \DIFadd{, we have conducted adiabatic, constant volume
...
low-temperature side of the predicted NTC. It is clear from \mbox{%DIFAUXCMD
\cref{fig:conv-comparison} }\hspace{0pt}%DIFAUXCMD
that the
model of \mbox{%DIFAUXCMD
\citet{Dievart2013} }\hspace{0pt}%DIFAUXCMD
predicts the NTC to occur at too high a temperature relative to the
experiments.
}\DIFaddend 
...
2.0\) data, the RMG model tends to over-predict the low-temperature ignition delays \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{(i.e., those to
the right of the experimental NTC region on the Arrhenius plot), }\DIFaddend and does not predict the NTC region
...
for the two models. The percentages shown in \DIFdelbegin \DIFdel{the }\DIFdelend \cref{tab:mv-radicals} are the percent of the fuel
...
\DIFdelbegin \DIFdel{This analysis shows }\DIFdelend \DIFaddbegin \DIFadd{The results of the analysis of the ``RMG switched'' model show }\DIFaddend that the radical on the fourth carbon
\DIFdelbegin \DIFdel{atom }\DIFdelend is still the most \DIFdelbegin \DIFdel{prevalant, despite changing }\DIFdelend \DIFaddbegin \DIFadd{prevalent, despite }\DIFaddend the heats of formation \DIFdelbegin \DIFdel{of the fuel and its radicals .
}\DIFdelend \DIFaddbegin \DIFadd{for the radicals in the ``RMG switched''
model being identical to the \mbox{%DIFAUXCMD
\citet{Dievart2013} }\hspace{0pt}%DIFAUXCMD
model. This suggests that the reaction pathways
