\documentclass[letterpaper, review, sort&compress]{elsarticle}
%DIF LATEXDIFF DIFFERENCE FILE
%DIF DEL methyl-valerate_old.tex   Mon Sep 25 14:55:53 2017
%DIF ADD methyl-valerate.tex       Wed Sep 27 12:12:58 2017
...
\ce{C14} to \ce{C20}~\cite{Hoekman2012}. Recognizing that the large molecular size of the methyl
...
Methyl esters larger than MB, such as methyl valerate (MV, \ce{C6H12O2}, methyl pentanoate), have
...
out to provide additional information on the fidelity of using MV as a biodiesel surrogate \DIFaddbegin \DIFadd{and
insights into the autoignition chemistry of large methyl esters}\DIFaddend .
...
\begin{center}
    ...
    \(\tau=\SI{27.56\pm0.89}{\ms}\), and \(\tau_1=\SI{16.60\pm0.46}{\ms}\). \DIFaddbegin \DIFadd{The non-reacting
    pressure trace by replacing \ce{O2} with \ce{N2} is also shown for reference.}\DIFaddend }
...
In addition to the reactive experiments, non-reactive experiments are conducted by replacing \ce{O2}
with \ce{N2} to determine the influence of machine-specific behavior on the experimental conditions
\DIFaddbegin \DIFadd{(see \mbox{%DIFAUXCMD
\cref{fig:ign-delay-def}}\hspace{0pt}%DIFAUXCMD
...
MV in ``air'' are considered. While \ce{O2} is kept at \SI{21}{\percent} by mole in the oxidizer,
...
pressure transducer. The same transducer is used to measure the pressure of the \DIFdelbegin \DIFdel{reactants }\DIFdelend \DIFaddbegin \DIFadd{reactant mixture
}\DIFaddend prior to an experiment.
...
so the reactants are well mixed for the duration of the experiments. \DIFaddbegin \DIFadd{Previous work has shown this
procedure to completely vaporize the fuel and prevent fuel cracking during the heating
process~\mbox{%DIFAUXCMD
\cite{Weber2011a,Kumar2009,Das2012}}\hspace{0pt}%DIFAUXCMD
.
}\DIFaddend 

\DIFdelbegin %DIFDELCMD < \begin{center}
%DIFDELCMD <     \captionsetup{type=figure}
%DIFDELCMD <     \input{figures/vapor_pressure.pgf}
%DIFDELCMD <     %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdel{Saturated vapor pressure of MV as a function of temperature,
    plotted using the Antoine equation, \mbox{%DIFAUXCMD
\cref{eq:antoine}}\hspace{0pt}%DIFAUXCMD
, with
    \(A=6.4030\), \(B=1528.69\), and \(C=52.881\).}}
    %DIFAUXCMD
%DIFDELCMD < \label{fig:vapor-pressure}
%DIFDELCMD < \end{center}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend The initial temperature is chosen such that the saturated vapor pressure \((P_{\text{sat}})\) of the
...
\(A\), \(B\), and \(C\) are substance-specific coefficients\DIFaddbegin \DIFadd{, given in units of \si{\kelvin} and
\si{\kPa}}\DIFaddend . Coefficients for \cref{eq:antoine} are given in the literature by~\citet{Ortega2003},
...
SciPy~\cite{Jones2001} version 0.19.0. \Cref{fig:vapor-pressure} shows that the coefficients fitted
with this procedure give good agreement with the experimental data; values for the coefficients
computed in this work and \DIFaddbegin \DIFadd{reported }\DIFaddend in the literature works are given in \cref{tab:antoine}. The data
and code used to calculate the coefficients are provided in the Supplementary Material.

\begin{center}
    \DIFaddbegin \captionsetup{type=figure}
    \input{figures/vapor_pressure.pgf}
    \caption{\DIFadd{Saturated vapor pressure of MV as a function of temperature,
    plotted using the Antoine equation, \mbox{%DIFAUXCMD
\cref{eq:antoine}}\hspace{0pt}%DIFAUXCMD
, with
    \(A=6.4030\), \(B=1528.69\), and \(C=52.881\).}}
...
\begin{center}
    \DIFaddend \captionsetup{type=table}
    \caption{Antoine Equation coefficients computed in this work and \DIFaddbegin \DIFadd{obtained }\DIFaddend from the literature\DIFaddbegin \DIFadd{,
    in units of \si{\kelvin} and \si{\kPa}}\DIFaddend . The $2\sigma$ confidence interval is estimated by taking
...
input files for the \DIFdelbegin \DIFdel{RMG }\DIFdelend \DIFaddbegin \DIFadd{RMG-generated }\DIFaddend model are available in the Supplementary Material.
...
the ignition delay data is available as a comma-separated value file in the Supplementary Material\DIFaddbegin \DIFadd{,
including the mixture conditions for each experiment, the initial conditions, the compressed
conditions, and the ignition delays and their associated errors}\DIFaddend .

\DIFaddbegin \begin{center}
    \captionsetup{type=figure}
    \includegraphics{figures/ignition-delays.pdf}
    \caption{\DIFadd{Ignition delays of MV as a function of inverse temperature for varying equivalence
    ratios. Filled points are the overall ignition delays and hollow points are the first stage
    ignition delays. a) \mbox{%DIFAUXCMD
\SI{15}{\bar}}\hspace{0pt}%DIFAUXCMD
, b) \mbox{%DIFAUXCMD
\SI{30}{\bar}}\hspace{0pt}%DIFAUXCMD
.}}
    \label{fig:ignition-delays}
\end{center}

\DIFaddend It can be seen from \cref{fig:ignition-delays} that the ignition delays for the \(\phi=0.25\text{
...
\DIFdelbegin %DIFDELCMD < \begin{center}
%DIFDELCMD <     \captionsetup{type=figure}
%DIFDELCMD <     \includegraphics{figures/ignition-delays.pdf}
%DIFDELCMD <     %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
...
.}}
    %DIFAUXCMD
%DIFDELCMD < \label{fig:ignition-delays}
%DIFDELCMD < \end{center}
%DIFDELCMD < 
...
hydroperoxyalkyl radical (\ce{QOOH}) and \ce{O2} becoming more dominant than the unimolecular
decomposition of \ce{QOOH} at higher pressures. Similar trends could occur for the hydroperoxyalkyl
radicals of MV.
...
\begin{center}
    ...
    \caption{Selected pressure traces around the NTC region of ignition delay.  a) \DIFdelbegin \DIFdel{\(\phi=1.0,\ P_C=\SI{30}{\bar}\)}\DIFdelend \DIFaddbegin \DIFadd{\(\phi=1.0\)}\DIFaddend ,
    \DIFaddbegin \DIFadd{\(P_C=\SI{30}{\bar}\), }\DIFaddend b) \DIFdelbegin \DIFdel{\(\phi=2.0,\ P_C=\SI{15}{\bar}\)}%DIFDELCMD < }%%%
\DIFdelend \DIFaddbegin \DIFadd{\(\phi=2.0\), \(P_C=\SI{15}{\bar}\)}\DIFaddend . \DIFaddbegin \DIFadd{The corresponding non-reactive
    pressure traces are also included for reference.}}
    \DIFaddend \label{fig:pressure-traces}
\end{center}
...
\begin{center}
    ...
    \caption{Comparison of experimental \DIFaddbegin \DIFadd{(\(\tau\) }\DIFaddend and \DIFaddbegin \DIFadd{\(\tau_1\)) and }\DIFaddend simulated \DIFdelbegin \DIFdel{results}\DIFdelend \DIFaddbegin \DIFadd{(\(\tau\)) ignition
    delays computed using the procedure described in \mbox{%DIFAUXCMD
\cref{sec:experimental-modeling}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . a)
    \SI{15}{\bar}, b) \SI{30}{\bar}\DIFdelbegin %DIFDELCMD < }%%%
\DIFdelend .\DIFaddbegin }
    \DIFaddend \label{fig:simulation-comparison}
\end{center}

At \SI{15}{\bar}, the experimental \DIFaddbegin \DIFadd{overall }\DIFaddend ignition delays are under-predicted by the
\citet{Dievart2013} model for the three equivalence ratios shown. For the \(\phi =\)
\numlist{0.25;0.5} conditions, the model appears to be predicting an NTC region of the \DIFaddbegin \DIFadd{overall
}\DIFaddend ignition delays as the temperature decreases, \DIFaddbegin \DIFadd{as judged by the increasing curvature of the
simulations, }\DIFaddend although such a trend is not observed \DIFdelbegin \DIFdel{in the }\DIFdelend \DIFaddbegin \DIFadd{for the experimental }\DIFaddend data. However, at \(\phi =
...
similar trend as the \DIFdelbegin \DIFdel{\(\phi=2.0,\
P_C=\SI{15}{\bar}\) }\DIFdelend \DIFaddbegin \DIFadd{\(\phi=2.0\), \(P_C=\SI{15}{\bar}\) }\DIFaddend data. The model does not predict the NTC
region found experimentally for the \(\phi = 1.0\)\DIFaddbegin \DIFadd{, \(P_C=\SI{30}{\bar}\) }\DIFaddend experiments, but the
...
\DIFdelbegin \DIFdel{While }\DIFdelend \DIFaddbegin 

\DIFadd{To further understand }\DIFaddend the model of \citet{Dievart2013}\DIFdelbegin \DIFdel{over-predicts the first-stage ignition delay }\DIFdelend \DIFaddbegin \DIFadd{, we have conducted adiabatic, constant volume
...
simulations, overall ignition delay is defined as an increase in the temperature of \mbox{%DIFAUXCMD
\SI{400}{\K}
...
\mbox{%DIFAUXCMD
\cref{tab:mixtures} }\hspace{0pt}%DIFAUXCMD
...
\begin{center}
    ...
    \caption{\DIFadd{Comparison of simulated overall ignition delays computed in an adiabatic, constant
    volume system (dotted lines) and computed using the procedure described in
    \mbox{%DIFAUXCMD
\cref{sec:experimental-modeling} }\hspace{0pt}%DIFAUXCMD
(solid lines). a) \mbox{%DIFAUXCMD
\SI{15}{\bar}}\hspace{0pt}%DIFAUXCMD
, b) \mbox{%DIFAUXCMD
\SI{30}{\bar}}\hspace{0pt}%DIFAUXCMD
.}}
...
\Cref{fig:conv-comparison} }\hspace{0pt}%DIFAUXCMD
compares the CONV simulations to the RCM simulations shown in
\mbox{%DIFAUXCMD
\cref{fig:simulation-comparison}}\hspace{0pt}%DIFAUXCMD
. From \mbox{%DIFAUXCMD
\cref{fig:conv-comparison}}\hspace{0pt}%DIFAUXCMD
, it can be seen that the curvature
in the RCM simulations at \(P_C = \SI{15}{\bar}\), \(\phi =\) \mbox{%DIFAUXCMD
\numlist{0.25;0.5} }\hspace{0pt}%DIFAUXCMD
is related to the
NTC region of the overall ignition delay, while the lack of curvature in the \(P_C =
\SI{15}{\bar}\)}\DIFaddend , \DIFdelbegin \DIFdel{it also
over-predicts the first-stage pressure rise, thereby leading to rapid overall ignition .
}\DIFdelend \DIFaddbegin \DIFadd{\(\phi = \num{2.0}\) and the \(P_C = \SI{30}{\bar}\), \(\phi = \num{1.0}\)
simulations is because those lie on the low-temperature side of the predicted NTC. It is clear from
\mbox{%DIFAUXCMD
\cref{fig:conv-comparison} }\hspace{0pt}%DIFAUXCMD
that the model of \mbox{%DIFAUXCMD
\citet{Dievart2013} }\hspace{0pt}%DIFAUXCMD
predicts the NTC to occur at too
...
the data, we \DIFdelbegin \DIFdel{constructed }\DIFdelend \DIFaddbegin \DIFadd{conduct simulations with }\DIFaddend an additional model \DIFaddbegin \DIFadd{constructed }\DIFaddend using RMG (see
...
\citet{Dievart2013} model for the \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\SI{15}{\bar}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\(P_C = \SI{15}{\bar}\)}\DIFaddend , \(\phi =\) \numlist{0.25; 0.5} data.
Moreover, the RMG model predicts a similar NTC region as temperature is decreasing. For the \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\SI{15}{\bar}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\(P_C =
\SI{15}{\bar}\)}\DIFaddend , \(\phi = 2.0\) data, the RMG model tends to over-predict the low-temperature
\DIFdelbegin \DIFdel{ignition delays, }\DIFdelend \DIFaddbegin \DIFadd{overall ignition delays (i.e., those to the right of the experimental NTC region on the Arrhenius
plot), }\DIFaddend and does not predict the NTC region found experimentally. As before, the trend at \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\SI{30}{\bar}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\(P_C =
\SI{30}{\bar}\)}\DIFaddend , \(\phi=1.0\) is similar to the \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\SI{15}{\bar}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\(P_C = \SI{15}{\bar}\)}\DIFaddend , \(\phi=2.0\) data; the RMG
model over-predicts the low-temperature \DIFaddbegin \DIFadd{overall }\DIFaddend ignition delays and does not predict the
...
delay.
\DIFdelbegin \DIFdel{Again, the over-prediction of the first-stage ignition delay and pressure rise requires
...
reactive), and so these conditions \DIFdelbegin \DIFdel{aren't }\DIFdelend \DIFaddbegin \DIFadd{are not }\DIFaddend compared in \cref{fig:simulation-comparison}. We note
...
THERM\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Ritter1991} }\hspace{0pt}%DIFAUXCMD
is }\DIFdelend \DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{Ritter1991} }\hspace{0pt}%DIFAUXCMD
...
\Cref{tab:heat-of-formation} }\hspace{0pt}%DIFAUXCMD
...
\DIFaddend \begin{center}
    ...
    \begin{tabular}{cSSSS}
        \toprule
        \multirow{2}{*}{Radical Site} & \multicolumn{2}{c}{\citet{Dievart2013}} & \multicolumn{2}{c}{RMG Model (this work)} \\
...
initial temperature and pressure of \SI{700}{\K} \DIFdelbegin \DIFdel{, \mbox{%DIFAUXCMD
\SI{30}{\bar}}\hspace{0pt}%DIFAUXCMD
...
\DIFdelbegin %DIFDELCMD < \begin{center}
%DIFDELCMD <     \captionsetup{type=table}
%DIFDELCMD <     %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdel{Percent of MV consumed to form fuel radical species with a hydrogen atom missing at
    the location indicated in the first column and \mbox{%DIFAUXCMD
\cref{fig:mv-structure}}\hspace{0pt}%DIFAUXCMD
}}
    %DIFAUXCMD
%DIFDELCMD < \label{tab:mv-radicals}
%DIFDELCMD <     \begin{tabular}{cSSS}
%DIFDELCMD <         \toprule
%DIFDELCMD <         %%%
\DIFdel{Radical Site }%DIFDELCMD < & {%%%
\DIFdel{\mbox{%DIFAUXCMD
\citet{Dievart2013} }\hspace{0pt}%DIFAUXCMD
}%DIFDELCMD < [%%%
\DIFdel{\si{\percent}}%DIFDELCMD < ]} & {%%%
\DIFdel{RMG Model }%DIFDELCMD < [%%%
\DIFdel{\si{\percent}}%DIFDELCMD < ]} & {%%%
\DIFdel{RMG switched }%DIFDELCMD < [%%%
\DIFdel{\si{\percent}}%DIFDELCMD < ]}\\
%DIFDELCMD <         \midrule
%DIFDELCMD <         %%%
\DIFdel{2 }%DIFDELCMD < & %%%
\DIFdel{29.2 }%DIFDELCMD < & %%%
\DIFdel{12.5 }%DIFDELCMD < & %%%
\DIFdel{11.0 }%DIFDELCMD < \\
%DIFDELCMD <         %%%
\DIFdel{3 }%DIFDELCMD < & %%%
\DIFdel{17.5 }%DIFDELCMD < & %%%
\DIFdel{12.2 }%DIFDELCMD < & %%%
\DIFdel{11.1 }%DIFDELCMD < \\
%DIFDELCMD <         %%%
\DIFdel{4 }%DIFDELCMD < & %%%
\DIFdel{17.5 }%DIFDELCMD < & %%%
\DIFdel{50.6 }%DIFDELCMD < & %%%
\DIFdel{56.6 }%DIFDELCMD < \\
%DIFDELCMD <         %%%
\DIFdel{5 }%DIFDELCMD < & %%%
\DIFdel{9.5 }%DIFDELCMD < & %%%
\DIFdel{3.9 }%DIFDELCMD < & %%%
\DIFdel{4.3 }%DIFDELCMD < \\
%DIFDELCMD <         %%%
\DIFdel{M }%DIFDELCMD < & %%%
\DIFdel{26.3 }%DIFDELCMD < & %%%
\DIFdel{20.8 }%DIFDELCMD < & %%%
\DIFdel{16.9 }%DIFDELCMD < \\
...
The model of \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Dievart2013} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{Dievart2013} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend predicts that \ce{H}-abstraction from the second carbon is the most
...
\DIFaddbegin \begin{center}
    \captionsetup{type=table}
    \caption{\DIFadd{Percent of MV consumed to form fuel radical species with a hydrogen atom missing at
    the location indicated in the first column and \mbox{%DIFAUXCMD
\cref{fig:mv-structure}}\hspace{0pt}%DIFAUXCMD
}}
    ...
    \begin{tabular}{cSSS}
        \toprule
        \DIFadd{Radical Site }& {\DIFadd{\mbox{%DIFAUXCMD
\citet{Dievart2013} }\hspace{0pt}%DIFAUXCMD
}[\DIFadd{\si{\percent}}]} & {\DIFadd{RMG Model }[\DIFadd{\si{\percent}}]} & {\DIFadd{RMG switched }[\DIFadd{\si{\percent}}]}\\
        \midrule
        \DIFadd{2 }& \DIFadd{29.2 }& \DIFadd{12.5 }& \DIFadd{11.0 }\\
        \DIFadd{3 }& \DIFadd{17.5 }& \DIFadd{12.2 }& \DIFadd{11.1 }\\
        \DIFadd{4 }& \DIFadd{17.5 }& \DIFadd{50.6 }& \DIFadd{56.6 }\\
        \DIFadd{5 }& \DIFadd{9.5 }& \DIFadd{3.9 }& \DIFadd{4.3 }\\
        \DIFadd{M }& \DIFadd{26.3 }& \DIFadd{20.8 }& \DIFadd{16.9 }\\
        \bottomrule
    \end{tabular}
\end{center}

\DIFaddend To further compare the models with each other, the NASA polynomials representing the thermodynamic
properties of MV and the 5 fuel radicals from the model of \citet{Dievart2013} \DIFdelbegin \DIFdel{and }\DIFdelend are used to replace
the equivalent molecules in the RMG model. The results of a path analysis at the same condition as
the other analysis is shown in \cref{tab:mv-radicals} in the ``RMG switched'' column. \DIFdelbegin \DIFdel{This analysis shows }\DIFdelend \DIFaddbegin \DIFadd{The results of
the analysis of the ``RMG switched'' model show }\DIFaddend that the radical on the fourth carbon \DIFdelbegin \DIFdel{atom }\DIFdelend is still the
most \DIFdelbegin \DIFdel{prevalant, despite changing }\DIFdelend \DIFaddbegin \DIFadd{prevalent, despite }\DIFaddend the heats of formation \DIFdelbegin \DIFdel{of the fuel and its radicals .
}\DIFdelend \DIFaddbegin \DIFadd{for the fuel radicals in the ``RMG switched'' model
being identical to the \mbox{%DIFAUXCMD
\citet{Dievart2013} }\hspace{0pt}%DIFAUXCMD
model. This suggests that the reaction pathways have a
...
to a single source. \DIFdelbegin \DIFdel{There is a strong interaction between the thermodynamics of the species and the
kinetics of the reactions, requiring }\DIFdelend \DIFaddbegin \DIFadd{Separating the influence of thermochemistry and kinetics requires }\DIFaddend further
detailed study of the methyl \DIFdelbegin \DIFdel{ester system }\DIFdelend \DIFaddbegin \DIFadd{valerate system specifically, and methyl ester systems more generally.
Although such detailed work has begun, for example, with the work of \mbox{%DIFAUXCMD
\citet{Hayes2009}}\hspace{0pt}%DIFAUXCMD
, further work
is required }\DIFaddend to accurately predict the low temperature ignition delays of methyl valerate.

\section{Conclusions}\label{sec:conclusions}

In this study, we have measured ignition delays for methyl valerate over a wide range of
engine-relevant pressures, temperatures, and equivalence ratios. An NTC region of the \DIFaddbegin \DIFadd{overall
}\DIFaddend ignition delay and two-stage ignition are recorded for pressures of \SI{15}{\bar} at \(\phi=2.0\)
...
Both models predict an NTC region of the \DIFaddbegin \DIFadd{overall }\DIFaddend ignition delay under conditions where none is found
in the experiments, and fail to predict the NTC region of \DIFaddbegin \DIFadd{overall }\DIFaddend ignition delay that is present in
